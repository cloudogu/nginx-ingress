...
.PHONY: docker-build
docker-build: ## Builds the docker image of the dogu with the name.
...
${K8S_CLUSTER_ROOT}/image.tar:
	# Saves the image into a file into the K8s root path to be available on all nodes.
	docker save ${IMAGE} -o ${K8S_CLUSTER_ROOT}/image.tar

.PHONY: image-import
image-import: ${K8S_CLUSTER_ROOT}/image.tar ## Imports the currently available image into the K8s cluster for all nodes.
